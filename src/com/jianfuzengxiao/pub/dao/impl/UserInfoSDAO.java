...
public class UserInfoSDAO extends BaseDAO<UserInfoMVO> implements IUserInfoSDAO {
	...
	public UserInfoMVO insert(final UserInfoMVO entity) throws SysException {
		...
		sql.append(
				"INSERT INTO USER_INFO (userid,username,gender,face_photo,face_file,birth_date,nation_id,nation_name,telephone,cert_type_id,cert_type_name,cert_number,cert_start_time,cert_stop_time,cert_address,cert_office,status,audit_remark,create_time,update_time,sts) ");
		sql.append(
	...
	public int delete(UserInfoMVO entity) throws SysException {
		...
		try {
			logger.info(sql.toString());
			rowsAffected = jdbcTemplate.update(sql.toString(), entity.getUserid());
		} catch (DataAccessException e) {
