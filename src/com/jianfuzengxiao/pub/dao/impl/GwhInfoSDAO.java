...
public class GwhInfoSDAO extends BaseDAO<GwhInfoMVO> implements IGwhInfoSDAO {
	...
	public GwhInfoMVO insert(final GwhInfoMVO entity) throws SysException {
		...
		sql.append(
				"INSERT INTO GWH_INFO (gwh_id,gwh_name,list_order,prov_code,city_code,area_code,create_time,update_time,sts) ");
	...
	public int delete(GwhInfoMVO entity) throws SysException {
		...
		try {
			logger.info(sql.toString());
			rowsAffected = jdbcTemplate.update(sql.toString(), entity.getGwhId());
		} catch (DataAccessException e) {
	...
	public List<GwhInfoMVO> queryList(GwhInfoMVO entity) throws SysException {
		StringBuilder sql = new StringBuilder();
		sql.append(
				"SELECT gwh_id,gwh_name,list_order,prov_code,city_code,area_code,date_format(create_time,'%Y-%m-%d %H:%i:%s')create_time,date_format(update_time,'%Y-%m-%d %H:%i:%s')update_time,sts ");
