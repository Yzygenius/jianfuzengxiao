...
public class UserInfoMDAO extends UserInfoSDAO implements IUserInfoMDAO {
	...
	public PageInfo queryPage(UserInfoMVO entity, PageInfo pageInfo) throws SysException {
		...
		sql.append(
				"select user_id,op_id,username,gender,face_photo,face_file,date_format(birth_date,'%Y-%m-%d %H:%i:%s')birth_date,nation_id,nation_name,telephone,certificates_type_id,certificates_type_name,certificates_number,certificates_start_time,certificates_stop_time,certificates_address,certificates_office,status,audit_remark,date_format(create_time,'%Y-%m-%d %H:%i:%s')create_time,date_format(update_time,'%Y-%m-%d %H:%i:%s')update_time,sts,date_format(lease_start_time,'%Y-%m-%d %H:%i:%s')lease_start_time ");
