package com.jianfuzengxiao.pub.dao;

import com.bamboo.framework.entity.PageInfo;
import com.bamboo.framework.exception.SysException;
import com.jianfuzengxiao.pub.entity.CommunityInfoMVO;
import com.jianfuzengxiao.pub.entity.HousesInfoMVO;
import com.jianfuzengxiao.pub.entity.LgzgMVO;

public interface ILgzgMDAO extends ILgzgSDAO {
	/** 分页查询 */
	public PageInfo queryPage(LgzgMVO lzzg, PageInfo pageInfo) throws SysException;

	public PageInfo queryManageCommunityPage(LgzgMVO lzzg, PageInfo pageInfo) throws SysException;
	
	public PageInfo queryPageNotAdminCommuntiy(CommunityInfoMVO entity, PageInfo pageInfo) throws SysException;
}
