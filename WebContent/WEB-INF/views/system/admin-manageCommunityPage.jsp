...
<body>
	...
	<div class="x-body">
		...
		<xblock>
			...
			<button class="layui-btn"
				onclick="banner_add('添加流管专干管辖社区权限','/jianfuzengxiao/system/lgzg/toChoosePage.html')">
	...
	<script>
		...
		function page() {
			...
			$.ajax({
				url : "/jianfuzengxiao/system/lgzg/getManageCommunityPage.html",
				type : 'post',
				dataType : "json",
				data: data,
				success : function(result) {
				...
				success : function(result) {
					if (result.code == 1) {
						...
						for (var i = 0; i < data.length; i++) {
							...
							tr.find('[row=checkBoxId]').children().val(data[i].lgzgId);
							tr.find('[row=ids]').text(data[i].lgzgId);
							tr.find('[row=ids2]').text(data[i].communityId);
		...
		function delAll() {
			layer.confirm('确认要删除吗？', function(index) {
				...
				var sel = arr.join(",");
				$.ajax({
					url : "/jianfuzengxiao/system/lgzg/delCommunity.html",
					type : 'post',
					dataType : "json",
					data : {
						'lgzgId' : sel
