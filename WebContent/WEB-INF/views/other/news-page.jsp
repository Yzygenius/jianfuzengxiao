...
<body>
	<div class="x-nav">
		<span class="layui-breadcrumb"> <a><cite>首页</cite></a> <a><cite>资讯列表</cite></a>
	...
	<div class="x-body">
		...
		<button class="layui-btn"
			onclick="banner_add('新增','/tchyuanlin/system/news/toNewsAdd.html')">
	...
	<table id="clone-tr" style="display:none;">
		<tr>
			...
			<td row="listOrder"></td>
			<td row="title"><div style="width:200px;height:22px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"></div></td>
			...
			<td class="td-manage">
				<button class="layui-btn layui-btn layui-btn-xs"
					onclick="banner_details(this,'查看','/tchyuanlin/system/news/toNewsDetails.html')">
					<i class="layui-icon">&#xe642;</i>查看
				</button>
				<button class="layui-btn layui-btn layui-btn-xs"
					onclick="banner_edit(this,'编辑','/tchyuanlin/system/news/toNewsEdit.html')">
...
$(function () {
    layui.use(['laydate','form','element','laypage','layer'], function(){
      ...
      $.ajax({  
		  url : "/tchyuanlin/system/news/getNewsPage.html",  
		  type : 'post',
		  dataType: "json",
   		...
   		$.ajax({  
			...
			success : function(result){
				...
				for(var i=0;i<data.length;i++){
					...
					tr.find('[row=checkBoxId]').children().val(data[i].newsId);
    ...
    //批量删除提交
     function delAll () {
        layer.confirm('确认要删除吗？',function(index){
        	...
        	var sel = arr.join(",");
        	$.ajax({  
 			url : "/tchyuanlin/system/news/updateStsNews.html",  
 			type : 'post',
 			dataType: "json",
 			data: {'newsId': sel},
 			success : function(result){
    ...
    function banner_details(obj,title,url,w,h){
    	var newsId = $(obj).parent('td').siblings('[row=checkBoxId]').children('input').val();
    	x_admin_show(title,url+'?newsId='+newsId,w,h); 
    }
 	// 编辑
