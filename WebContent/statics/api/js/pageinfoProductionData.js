...
//var page = 1;
/* 当前页数， 总页数 */
function pageinfoProductionData(page, pageCount,element){
    ...
    if (pageCount <= pageGeShu) {
        ...
        if(page == 1){
            ...
            for(var i = 0; i < pageCount; i++){
                ...
                var p = (i + 1);
                if(i == 0){
                    str += '<span class="active" onclick="getProduceDataInfo('+p+')">'+p+'</span>';
                }else{
                    str += '<span onclick="getProduceDataInfo('+p+')">'+p+'</span>';
                }
            }
	...
	$(parentElement).find('.pageRightDouble').click(function(){
	    var li = $(parentElement).find('.pageNumbers').find('.active').text();
	    if(pageCountData > li){
	    	getProduceDataInfo(pageCountData)
	    }else{
	        //alert('禁用')
	    }
	});
