...
Button.prototype.iniButton = function(){
    ...
    function mouseLeave(){
		...
		if(thisObj.changePos == true){
			...
			//背景图
			thisObj.btn.css("background", "url('/jikuang/statics/api/images/ui/button_"+thisObj.type+"_"+thisObj.size.size+"_"+"normal.png') no-repeat 100% 100%");
		}
	}
    function mouseDown(){
    	...
    	thisObj.label.offset({top:thisObj.label.offset().top+2,left:thisObj.label.offset().left+2});
        //背景图
    	thisObj.btn.css("background", "url('/jikuang/statics/api/images/ui/button_"+thisObj.type+"_"+thisObj.size.size+"_"+"press.png') no-repeat 100% 100%");
    }
    function mouseUp(){
		thisObj.btn.unbind("mouseleave",mouseLeave);
		if(thisObj.changePos == true){
...
Button.prototype.setCss = function(){
    ...
    css = {
		...
		"outline": "none",
		"color": this.color,
    	"background": "url('/jikuang/statics/api/images/ui/button_"+this.type+"_"+this.size.size+"_"+"normal.png') no-repeat 100% 100%",
    }
	this.btn.css(css);
	css = {
...
Button.prototype.setDisable = function(disableFlg){ //设置是否可用
    ...
    if(disableFlg){
        ...
        this.btn.css({"background": "url('/jikuang/statics/api/images/ui/button_disable_"+this.size.size+".png') no-repeat 100% 100%"});
    }else{
		this.btn.css({"color":this.color});
        this.btn.css({"background": "url('/jikuang/statics/api/images/ui/button_"+this.type+"_"+this.size.size+"_"+"normal.png') no-repeat 100% 100%"});
    }
}
