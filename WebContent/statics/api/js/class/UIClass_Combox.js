...
Combox.TypeProductType = "TypeProductType";//集矿产品类型
...
Combox.prototype.setCss = function(){
    ...
    css = {
        ...
        "height": "201px",
        "display": "none",
        "background": "url('/jikuang/statics/api/images/ui/combox_bg.png') no-repeat 100% 100%",
    }
...
Combox.prototype.getComboxData = function(typeFlg){
    ...
    else if(typeFlg==Combox.TypeProductType){
    	$.ajax({  
		    url : "/jikuang/api/goodsinfo/getGoodsInfoList.html",  
		    type : 'post',
		    dataType: "json",
		    success : function(data) {
		    	thisObj.comboxData.push({optionId:Combox.DefaultValue, optionName:"请选择"});
                for(var i=0; i<data.data.length; i++){
                    thisObj.comboxData.push({optionId: data.data[i].goodsId, optionName: data.data[i].productName});
                }
                //设置下拉框
                thisObj.setCombox(thisObj.comboxData);
		    }  
...
Combox.prototype.setSelect = function(optionId){ //设置选中
    ...
    for(var i=0; i<this.optionList.length; i++){
        if(this.optionList[i].attr("optionValue")==optionId){
            ...
            this.optionList[i].css("background","#fa6d35");
