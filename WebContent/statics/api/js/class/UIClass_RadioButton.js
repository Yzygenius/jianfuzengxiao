...
RadioButton.prototype.iniRadioButton = function(){
    ...
    function mouseLeave(){
		...
		if(thisObj.changePos == true){
			...
			//背景图
			thisObj.btn.css("background", "url('/jikuang/statics/api/images/ui/button_radio_mini_normal.png') no-repeat 100% 100%");
		}
	}
    function mouseDown(){
    	...
    	thisObj.btn.offset({top:thisObj.btn.offset().top+2,left:thisObj.btn.offset().left+2});
        //背景图
    	thisObj.btn.css("background", "url('/jikuang/statics/api/images/ui/button_radio_mini_press.png') no-repeat 100% 100%");
    }
    function mouseUp(){
		thisObj.btn.unbind("mouseleave",mouseLeave);
		if(thisObj.changePos == true){
...
RadioButton.prototype.setCss = function(){
    ...
    css = {
		...
		"background": "url('/jikuang/statics/api/images/ui/button_radio_mini_normal.png') no-repeat 100% 100%",
    }
	this.btn.css(css);
    css = {
        "margin-left": "10px",
...
RadioButton.prototype.setRadio = function(radioFlg){
	...
	if(this.radioClick){
		this.img.attr("src","/jikuang/statics/api/images/ui/radioFa.png");
	}else{
		this.img.attr("src","/jikuang/statics/api/images/ui/unradio.png");
	}
}
...
RadioButton.prototype.setDisable = function(disableFlg){ //设置是否可用
    ...
    if(disableFlg){
        ...
        this.btn.attr("src","/jikuang/statics/api/images/ui/unradio.png");
    }
}
